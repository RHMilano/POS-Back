...
namespace Tools.TryCatchI
{
    ...
    public class TryCatchBusinessExecutor
    {
        ...
        /// <returns></returns>
        public ResponseBussiness<T> SafeExecutor<T>(Func<T> action, string errMsg = "Client BBVAv2")
        {
            try
            {
                return action();
        ...
        /// <returns></returns>
        public ResponseBussiness<T> AddErrorLog<T>(string description, string stackTrace, string errMsg)
        {
        ...
        {
            string path = @"C:\PosMilano\LogsMilano\BBVA\";//ConfigurationManager.AppSettings["logPath"].ToString();
